...
- CHAPTER의 모든 글자는 1080×1350 캔버스 가장자리에서 최소 72px 안쪽에 배치됩니다.
- 글자의 기준점만 확인하지 않고 실제 글리프의 좌·우·위·아래 경계를 측정해 안전영역 안으로 자동 보정합니다.
- 긴 제목과 장소명은 글자 크기를 단계적으로 줄인 뒤에도 넘치면 말줄임표로 정리합니다.
- 상단은 마스트헤드와 시리즈 정보를 한 줄 레일로 묶고, 하단은 제목·장소·연도만 남겨 사진을 가리는 면적을 줄였습니다.
- 제목과 장소가 비어 있으면 임시 문구를 만들지 않아 사진이 그대로 드러납니다.
...
# 4. 캐러셀, 초안 저장, 타이포 실제 경계 테스트
python -X utf8 tests/test_studio_v2.py
```
