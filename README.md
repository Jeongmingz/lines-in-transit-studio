...
1. **모바일 피드 최적화 4:5 / 심리스 규격 렌더링**:
   ...
   - 스마트폰 화면(약 390px 폭)에서도 또렷하게 식별되는 **확대된 타이포그래피** (마스트헤드 48px, 타이틀 38px, SWIPE 배지 22px/175×56px)
2. **미리보기와 내보내기 캔버스 물리적 분리**:
   - 안전영역 및 50% 분할선은 순수 CSS 오버레이로 구현되어 출력 이미지에 절대 침범하지 않습니다.
   - 드래그 및 줌 조작 시 `requestAnimationFrame` 디바운싱을 통해 모바일에서도 버벅임 없는 60fps 인터랙션을 제공합니다.
3. **모바일 터치 핀치 투 줌 & 줌 슬라이더**:
   - 휠이 없는 모바일 기기를 위해 2점 터치 핀치 줌 제스처 지원
   - 툴바 내 1.0x~3.0x 명시적 슬라이더 및 +/- 버튼 제공
4. **로컬 번들링 서체 (Cormorant Garamond)**:
   - iOS, Android, Windows 등 모든 운영체제에서 동일한 세리프 조판을 보장하도록 로컬 woff2 서체 내장.
5. **가변 품질 자동 감축 인코딩 (Adaptive Quality Engine)**:
   - 렌더링 후 Blob 용량을 확인하여, 사용자가 지정한 목표 용량(1.0MB / 1.4MB / 2.0MB)을 초과할 경우 품질을 0.94에서 단계적으로 미세 조정합니다.
